...
PCIe transaction monitoring uses the same USB framing (preamble, channel,
length), with channel 1 carrying a TLP record:

::

    USB Frame (pcie_screamer)
    Offset  Size  Field
    ------  ----  -----------
    0x00    4     Preamble (0x5aa55aa5)
    0x04    4     Destination (channel ID, 1 = monitor)
    0x08    4     Length (bytes)
    0x0C    N     Payload (TLP record)

TLP record payload (32-byte header + data):
...
::
    ...
    Word 0 (64-bit)
      ...
      [15]    truncated (1 if payload truncated)
    ...
    Word 3 (64-bit)
      ...
      [8]     pasid_valid (TX)
      [28:9]  pasid (TX)
      [29]    privileged (TX) / status[0] (Cpl)
      [30]    execute (TX) / status[1] (Cpl)
      [31]    status[2] (Cpl)
      [47:32] cmp_id (Cpl)
      [59:48] byte_count (Cpl)

Payload data follows as ``payload_length`` DWORDs (4 bytes each).
...
.. code-block:: python
    ...
    from bsa_pcie_exerciser.common.protocol import parse_stream

    def decode_monitor_packets(data: bytes):
        """Decode USB monitor stream into TLPPacket objects."""
            ...
            while True:
                ...
                if n > 0:
                    for pkt in decode_monitor_packets(buf[:n]):
                        ...
                        table.add_row(
                            f"{pkt.timestamp_us / 1000:.3f}",
                            "OUT" if pkt.direction_str == "->" else "IN",
                            "WR" if pkt.is_write else "RD",
                            str(pkt.bar_hit),
                            f"0x{addr:08X}",
                            str(pkt.payload_length * 4)
                        )
